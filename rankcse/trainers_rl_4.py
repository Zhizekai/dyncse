...
# RL
from rankcse.Agent_3 import PolicyNet, Critic, ReplayMemory, optimize_model
...
class CLTrainer(Trainer):
    ...
    def evaluate(
        ...
        def batcher(params, batch):
            ...
            sentences = [
                s + " ." if s.strip()[-1] not in PUNCTUATION else s for s in sentences
            ]
            sentences = [
            ...
            with torch.no_grad():
                _, last_hidden_state = self.model(
                    **batch, output_hidden_states=True, return_dict=True, sent_emb=False
                )
    ...
    def train(
        ...
        if self.args.second_teacher_name_or_path is None:
            if "rank" in self.args.first_teacher_name_or_path:
                ...
                teacher_pooler = (
                    "cls_before_pooler"
                    if (
                        "simcse" in self.args.first_teacher_name_or_path
                        or "diffcse" in self.args.first_teacher_name_or_path
                    )
                    else "avg"
                )
                teacher = Teacher(
                    model_name_or_path=self.args.first_teacher_name_or_path,
                    pooler=teacher_pooler,
                )
            # rank
            sentence_vecs = torch.tensor(np.load(self.model_args.corpus_vecs)).to(
                teacher.device
            )
            ...
            second_pooler = (
                "cls_before_pooler"
                if (
                    "simcse" in self.args.second_teacher_name_or_path
                    or "diffcse" in self.args.second_teacher_name_or_path
                )
                else "avg"
            )
            second_teacher = Teacher(
                model_name_or_path=self.args.second_teacher_name_or_path,
                pooler=second_pooler,
            )
            sentence_vecs = torch.tensor(np.load(self.model_args.corpus_vecs)).to(
                first_teacher.device
            )
            # sentence_vecs = sentence_vecs.half()
            sentence_vecs = normalize(sentence_vecs, p=2.0, dim=1)
            sentence_vecs_2 = torch.tensor(
                np.load(self.model_args.second_corpus_vecs)
            ).to(second_teacher.device)
            # sentence_vecs = sentence_vecs.half()
            sentence_vecs_2 = normalize(sentence_vecs_2, p=2.0, dim=1)

            # RL
            policy_model1 = PolicyNet(2, 768, 42, 58, self.args.device).to(
                self.args.device
            )
            Critic_model1 = Critic(128, 2, 768).to(self.args.device)
            policy_model2 = PolicyNet(2, 768, 58, 42, self.args.device).to(
                self.args.device
            )
        ...
        RL_train = True
        # if RL_train:
        #     policy_model = PolicyNet(2, 768,self.args.device).to(self.args.device)
        # else:
        #     policy_model = torch.load("dqn_pong_model")
        # policy_model = PolicyNet(2, 768,self.args.device).to(self.args.device)
        ...
        # target_policy = policy_model
        # target_policy.load_state_dict(policy_model.state_dict())
        first_memory = ReplayMemory(MEMORY_SIZE)
        second_memory = ReplayMemory(MEMORY_SIZE)
        TARGET_UPDATE = 100
        step_counter = 0  # 初始化步骤计数器
        PSEUDO_EPISODE_LENGTH = 10  # 定义伪回合的长度
            ...
            for step, inputs in enumerate(epoch_iterator):
                ...
                with torch.no_grad():
                    ...
                    if teacher is not None:
                        ...
                        teacher_top1_sim_pred = (
                            cos(z1T.unsqueeze(1), z2T.unsqueeze(0)) / self.args.tau2
                        )
                        ...
                        if "rank" in self.args.first_teacher_name_or_path:
                            ...
                            first_teacher_vecs = first_teacher_vecs[
                                input_ids == self.tokenizer.mask_token_id
                            ]
                            first_teacher_vecs = normalize(
                                first_teacher_vecs, p=2.0, dim=1
                            )
                            first_teacher_vecs = first_teacher_vecs.view(
                                (batch_size, num_sent, first_teacher_vecs.size(-1))
                            )  # (bs, num_sent, hidden)
                            first_teacher_z1, first_teacher_z2 = (
                                first_teacher_vecs[:, 0],
                                first_teacher_vecs[:, 1],
                            )
                        else:
                        ...
                        dist1 = torch.mm(
                            first_teacher_z1, torch.transpose(sentence_vecs, 0, 1)
                        )
                        dist2 = torch.mm(
                            first_teacher_z2, torch.transpose(sentence_vecs, 0, 1)
                        )
                        if self.model_args.second_corpus_vecs is not None:
                            second_dist1 = torch.mm(
                                second_teacher_z1,
                                torch.transpose(sentence_vecs_2, 0, 1),
                            )
                            second_dist2 = torch.mm(
                                second_teacher_z2,
                                torch.transpose(sentence_vecs_2, 0, 1),
                            )
                        ...
                        first_teacher_top1_sim = first_teacher_top1_sim.to(
                            second_teacher_top1_sim.device
                        )
                        # teacher_top1_sim_pred = (self.args.alpha_ * first_teacher_top1_sim) + (
                        #             (1.0 - self.args.alpha_) * second_teacher_top1_sim)
                        sim_tensor1 = torch.cat(
                            [
                                first_teacher_top1_sim.unsqueeze(0),
                                second_teacher_top1_sim.unsqueeze(0),
                            ],
                            dim=0,
                        )
                ...
                if (step + 1) % self.args.gradient_accumulation_steps == 0 or (
                    ...
                    with torch.no_grad():
                        ...
                        value1_state = [
                            s.float().to(self.args.device) for s in value1_state
                        ]
                        if step < 7812:
                        ...
                        value2_state = [
                            s.float().to(self.args.device) for s in value2_state
                        ]
                        if step < 7812:
                    ...
                    if second_last_state is not None:
                        ...
                        second_next_state = [
                            s.float().to(self.args.device) for s in second_next_state
                        ]
                    ...
                    if step_counter >= PSEUDO_EPISODE_LENGTH:
                        optimize_model(
                            first_memory, policy_model1, Critic_model1, self.args.device
                        )
                        first_memory.clear()
                    ...
                    if step == 7800 and RL_train:
                        torch.save(policy_model1, "dqn_pong_model1")
                        torch.save(policy_model2, "dqn_pong_model2")
    ...
    def compute_loss(self, model, inputs, return_outputs=False):
        ...
        if num_sent == 3:
            ...
            temp1 = torch.cosine_similarity(
                z1, z2, dim=1
            )  # Cosine similarity of positive pairs
            temp2 = torch.cosine_similarity(
                z1, z3, dim=1
            )  # Cosine similarity of soft negative pairs
        ...
        def _get_ranks(x: torch.Tensor) -> torch.Tensor:
            ...
            for i in range(n):
                ranks[i][x_rank[i]] = torch.arange(d, dtype=torch.float).to(
                    ranks.device
                )
            return ranks
        ...
        if self.model_args.second_corpus_vecs is not None:
            ...
            cos_sim_baseE = (self.args.alpha_ * cos_sim_baseE) + (
                (1.0 - self.args.alpha_) * cos_second_sim_baseE
            )
        ...
        cos_sim_baseE_bound = (
            torch.logical_and(
                ...
                cos_sim_baseE >= self.model_args.baseE_sim_thresh_low,
            )
            .type(torch.float)
            .to(cos_sim.device)
        )
        mse = loss_fct_baseE(cos_sim * self.model_args.temp, cos_sim_baseE)
        loss_baseE = torch.sum(mse * cos_sim_baseE_bound) / (
            torch.sum(cos_sim_baseE_bound) + 1e-8
        )
        ...
        def get_environment_state(
            sim_tensor, inputs, z1, z2, cos_sim, encoder, distillation_loss_fct
        ):
            state = []

            soft_lable = sim_tensor

            # 拼接 z1 和 z2 成 embeddings_tensor
            ...
            first_kd_loss = distillation_loss_fct(
                first_teacher_top1_sim_pred.to(encoder.device), student_top1_sim_pred
            )
            second_kd_loss = distillation_loss_fct(
                second_teacher_top1_sim_pred.to(encoder.device), student_top1_sim_pred
            )

            # 使用 torch.cat 在维度0上拼接这两个标量值
            concatenated_loss = torch.cat(
                [first_kd_loss.unsqueeze(0), second_kd_loss.unsqueeze(0)], dim=0
            )

            # 如果希望将其形状改为 (1, 2)，可以使用 unsqueeze(0)
        ...
        if self.model_args.distillation_loss == "listnet":
            distillation_loss_fct = ListNet(
                self.model_args.tau2, self.model_args.gamma_
            )
        elif self.model_args.distillation_loss == "listmle":
            distillation_loss_fct = ListMLE(
                self.model_args.tau2, self.model_args.gamma_
            )
        with torch.no_grad():
            sim_tensor1 = inputs["sim_tensor1"]
            sim_tensor2 = inputs["sim_tensor2"]
            first_teacher_state = get_environment_state(
                sim_tensor1, inputs, z1, z2, cos_sim, encoder, distillation_loss_fct
            )
            second_teacher_state = get_environment_state(
                sim_tensor2, inputs, z1, z2, cos_sim, encoder, distillation_loss_fct
            )
            first_teacher_policy = inputs["policy_model1"]
            second_teacher_policy = inputs["policy_model2"]
            if steps_done < 7812:
                first_action, first_avg_probability = first_teacher_policy.take_action(
                    first_teacher_state
                )
                second_action, second_avg_probability = (
                    second_teacher_policy.take_action(second_teacher_state)
                )
            else:
                first_action = model.first_actions
                second_action = model.second_actions
        ...
        if first_action == 0 and second_action == 0:
            ...
            teacher_top1_sim_pred = (weight1 * first_teacher_top1_sim) + (
                weight2 * second_teacher_top1_sim
            )
            student_top1_sim_pred = cos_sim.clone()
            kd_loss = distillation_loss_fct(
                teacher_top1_sim_pred.to(encoder.device), student_top1_sim_pred
            )
        ...
        if self.model_args.loss_type == "hinge":
            ...
            model.second_rewards = -loss
            # if model.first_actions == 1:
            #     model.first_rewards = -loss
            # else:
            #     model.first_rewards = 0
            # if model.second_actions == 1:
            #     model.second_rewards = -loss
            # else:
            #     model.second_rewards = 0
            # loss = loss_o + self.model_args.baseE_lmb * loss_baseE
        else:
