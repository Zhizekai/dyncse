...
class PolicyNet(nn.Module):
    def __init__(self, num_teacher, embedding_length,pre_alpha,pre_beta,device):
        ...
        self.fc_alpha = nn.Linear(128, 1)  # 输出 alpha
        self.fc_beta = nn.Linear(128, 1)  # 输出 beta
    ...
    def forward(self, x1, x2, x3):
        ...
        # 输出两个参数 alpha 和 beta
        alpha = self.fc_alpha(scaled_out_reshaped)
        beta = self.fc_beta(scaled_out_reshaped)
        alpha = torch.relu(alpha).mean() + self.pre_alpha
        beta = torch.relu(beta).mean() + self.pre_beta
        weights = [alpha, beta]
    ...
    def take_action(self, state):
        ...
        if random.random() < self.epsilon:
            # 以 epsilon 的概率随机选择动作
            action = torch.tensor([random.uniform(0, 1)], dtype=torch.float32).to(self.device)
        else:
            # 否则根据概率分布采样动作
