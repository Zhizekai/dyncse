    ...
    def evaluate(
        ...
        def batcher(params, batch):
            ...
            with torch.no_grad():
                ...
                last_hidden_state = self.model.last_hidden_state.detach()
            ...
            for step, inputs in enumerate(epoch_iterator):
                ...
                with torch.no_grad():
                    ...
                    if teacher is not None:
                        ...
                        z1 = z1.to(torch.float)
                        z2 = z2.to(torch.float)
                        dist1 = torch.mm(z1, torch.transpose(sentence_vecs, 0, 1))
                        dist2 = torch.mm(z2, torch.transpose(sentence_vecs, 0, 1))
                        cos = nn.CosineSimilarity(dim=-1)
                        teacher_top1_sim_pred = cos(z1.unsqueeze(1), z2.unsqueeze(0)) / self.args.tau2
