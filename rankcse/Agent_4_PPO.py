...
# Transition = namedtuple('Transion', ('state','action', 'weights', 'reward', 'value'))
...
class ReplayMemory(object):
    ...
    def push(self, *args):
        ...
        # print("action",args[2])  # action tensor(0.4113, device='cuda:0') action tensor(0.3896, device='cuda:0')
        # input()
...
def optimize_model(memory, policy_net,policy_net_target, critic, critic_target, device, lr=1e-4):
    ...
    for _ in range(NUM_PPO_UPDATES):
        ...
        # 计算回报第二种方式
        # advantage = compute_advantage(gamma, 0.95,  range(len(reward_batch)), device)
        ...
        for state in batch.state:
            if isinstance(state, torch.Tensor):
                ...
                state = [s.float().to(device) for s in state]
            critic_value = critic(*state, action= None).unsqueeze(0)
...
class Critic(nn.Module):
    def __init__(self, input_dim, num_teacher, embedding_length, hidden_dim=[256, 128],output_dim=1, batch_size=128):
