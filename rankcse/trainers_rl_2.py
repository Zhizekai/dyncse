...
# RL
from rankcse.Agent_1 import PolicyNet,Critic,ReplayMemory,optimize_model
...
class CLTrainer(Trainer):
    ...
    def train(self, model_path: Optional[str] = None, trial: Union["optuna.Trial", Dict[str, Any]] = None):
        ...
        if self.args.second_teacher_name_or_path is None:
            ...
            if "rank" in self.args.second_teacher_name_or_path:
                second_teacher = AutoModel.from_pretrained( \
                    self.args.second_teacher_name_or_path \
                    )
                second_teacher = second_teacher.to(self.args.device)
            else:
            ...
            for step, inputs in enumerate(epoch_iterator):
                ...
                with torch.no_grad():
                    ...
                    if teacher is not None:
                        ...
                        if "rank" in self.args.second_teacher_name_or_path:
                            second_teacher_vecs = second_teacher( \
                                input_ids=input_ids, \
                                attention_mask=attention_mask, \
                                token_type_ids=token_type_ids \
                                ).last_hidden_state
                            second_teacher_vecs = second_teacher_vecs[input_ids == self.tokenizer.mask_token_id]
                            second_teacher_vecs = normalize(second_teacher_vecs, p=2.0, dim=1)
                            second_teacher_vecs = second_teacher_vecs.view(
                                (batch_size, num_sent, second_teacher_vecs.size(-1)))  # (bs, num_sent, hidden)
                            second_teacher_z1, second_teacher_z2 = second_teacher_vecs[:, 0], second_teacher_vecs[:, 1]
                        else:
                        ...
                        dist2 = torch.mm(first_teacher_z1, torch.transpose(sentence_vecs, 0, 1))
                        cos = nn.CosineSimilarity(dim=-1)
    ...
    def compute_loss(self, model, inputs, return_outputs=False):
        ...
        if num_sent == 3:
            ...
            loss_o += loss1 * lambda_  # Total loss
        ...
        if first_action == 0 and second_action == 0:
            ...
            # 计算权重，确保它们之和等于1
            total_probability = first_action*first_avg_probability + second_action*second_avg_probability
            weight1 = first_action*first_avg_probability / total_probability
            weight2 = second_action*second_avg_probability / total_probability
            ...
            #teacher_top1_sim_pred = (0.7 * first_teacher_top1_sim) + (0.3 * second_teacher_top1_sim)
            student_top1_sim_pred = cos_sim.clone()
        ...
        if self.model_args.loss_type == "hinge":
            ...
            # loss = loss_o + self.model_args.t_lmb * kd_loss
            # loss = loss_o + self.model_args.baseE_lmb * loss_baseE + self.model_args.t_lmb * kd_loss
            # loss = loss_o + self.model_args.baseE_lmb * loss_baseE
        else:
