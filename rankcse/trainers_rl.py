...
# RL
from rankcse.Agent import PolicyNet,actor,ReplayMemory,optimize_model
...
class CLTrainer(Trainer):
    ...
    def evaluate(
        ...
        self.model.rewards += 100 * rewards
    ...
    def train(self, model_path: Optional[str] = None, trial: Union["optuna.Trial", Dict[str, Any]] = None):
        ...
        PSEUDO_EPISODE_LENGTH = 125 # 定义伪回合的长度
        ...
        for epoch in range(epochs_trained, num_train_epochs):
            ...
            for step, inputs in enumerate(epoch_iterator):
                ...
                with torch.no_grad():
                    ...
                    inputs["policy_model"] = policy_model
                    inputs["steps_done"] = step
                    inputs["sim_tensor"] = sim_tensor
                ...
                if (step + 1) % self.args.gradient_accumulation_steps == 0 or (
                    ...
                    total_reward += model.rewards
                    if last_state is not None:
                        next_state = model.states
                        next_state = next_state
                        memory.push(last_state, action, weights, rewards)
                    last_state = model.states
                    step_counter += 1
                    if step_counter >= PSEUDO_EPISODE_LENGTH:
                        optimize_model(memory, policy_model, self.args.device)
                        step_counter = 0
                        memory.clear()
                        # RL
                    if step == 7800 and RL_train:
                        torch.save(policy_model, "dqn_pong_model")
    ...
    def compute_loss(self, model, inputs, return_outputs=False):
        ...
        def get_environment_state(inputs, z1, z2, cos_sim, encoder, distillation_loss_fct):
            state = []

            sim_tensor = inputs["sim_tensor"]
        ...
        with torch.no_grad():
            state = get_environment_state(inputs,z1,z2,cos_sim,encoder,distillation_loss_fct)
            policy_model = inputs["policy_model"]
            action, avg_probability = policy_model.take_action(state)
