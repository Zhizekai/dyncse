...
class CLTrainer(Trainer):
    ...
    def train(self, model_path: Optional[str] = None, trial: Union["optuna.Trial", Dict[str, Any]] = None):
        ...
        if self.args.second_teacher_name_or_path is None:
            ...
            policy_model2_params = policy_model2.state_dict()
            policy_model1.load_state_dict(policy_model2_params)
            policy_model1_target.load_state_dict(policy_model2_params)
            policy_model2_target.load_state_dict(policy_model2_params)

            Critic_model2_params = Critic_model2.state_dict()
            Critic_model1.load_state_dict(Critic_model2_params)
            Critic_model1_target.load_state_dict(Critic_model2_params)
            Critic_model2_target.load_state_dict(Critic_model2_params)
        ...
        PSEUDO_EPISODE_LENGTH = 10 # 伪章节长度
    ...
    def compute_loss(self, model, inputs, return_outputs=False):
        ...
        pooler_output, last_hidden_state = model(**inputs) # last_hidden_state torch.Size([384, 768]) pooler_output torch.Size([128, 3, 768])
        ...
        def get_environment_state(sim_tensor, inputs, z1, z2, cos_sim, encoder, distillation_loss_fct):
            ...
            x3 = concatenated_loss  # torch.Size([1, 2])
            state.append(x3)

            # print(x1.data.cpu().shape)
            # print(x2.data.cpu().shape)
            # print(x3.data.cpu().shape)

            ## 大概应该在这个位置加入对抗样本，并cat 进state数组里
            return state
